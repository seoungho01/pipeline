pipeline {
    ...
    stages {
        stage('github clone') {
            steps {
                git 'https://github.com/johrei1102/one-apigate.git'
            }
        }
        stage('build') {
            steps {
     ...
     //         sh 'mvn clean compile package -DskipTests=true'
        ...
        stage('deploy') {
            steps {
              deploy adapters: [tomcat9(credentialsId: 'deployer_user', path: '', url: 'http://192.168.2.1:8088/')], contextPath: null, war: '**/*.war'
            }
        }
        stage('ssh publisher') {
            steps {
